...
import { getMilestoneById, updateMilestone, deleteMilestone } from '@/lib/morgan/persistence';
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const item = await getMilestoneById(id, tenant.context);
    if (!item) return NextResponse.json({ error: 'Milestone not found.' }, { status: 404 });
    return NextResponse.json(item);
  } catch (error: unknown) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to load milestone.' }, { status: 500 });
  }
}
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await updateMilestone(id, body, tenant.context);
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!tenant.ok) return NextResponse.json({ error: tenant.error }, { status: tenant.status });
    const { id } = await params;
    const deleted = await deleteMilestone(id, tenant.context);
