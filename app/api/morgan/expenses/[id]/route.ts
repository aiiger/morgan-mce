...
import { getExpenseById, updateExpense, deleteExpense } from '@/lib/morgan/persistence';
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const item = await getExpenseById(id, tenant.context);
    if (!item) return NextResponse.json({ error: 'Expense not found.' }, { status: 404 });
    return NextResponse.json(item);
  } catch (error: unknown) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to load expense.' }, { status: 500 });
  }
}
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await updateExpense(id, body, tenant.context);
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!tenant.ok) return NextResponse.json({ error: tenant.error }, { status: tenant.status });
    const { id } = await params;
    const deleted = await deleteExpense(id, tenant.context);
