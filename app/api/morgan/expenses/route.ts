...
import { listExpenses, createExpense } from '@/lib/morgan/persistence';
...
export async function GET(request: Request) {
  try {
    ...
    const items = await listExpenses(tenant.context);
...
export async function POST(request: Request) {
  try {
    ...
    if (!tenant.ok) return NextResponse.json({ error: tenant.error }, { status: tenant.status });
    const created = await createExpense(body ?? {}, tenant.context);
