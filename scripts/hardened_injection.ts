...
async function main() {
    console.log('--- HARDENED INJECTION START ---');
    ...
    try {
        ...
        for (const project of payload.projects.slice(0, 5)) {
            ...
            // STEP 2: Project Manager (Keyboard Selection)
            console.log('Selecting PM...');
            // Find the dropdown that says "No user" or is under "Project Manager" label
            const pmDropdown = page.locator('.ant-select-selector:has-text("No user"), label:has-text("Project Manager") + div .ant-select-selector').first();
            ...
            if (await createBtn.isVisible()) {
                ...
                await page.screenshot({ path: `C:/Users/t1glish/.gemini/antigravity/brain/61544510-cff3-44bb-aa3d-ba480fb9dee7/hardened_fail_${Date.now()}.png` });
            }
